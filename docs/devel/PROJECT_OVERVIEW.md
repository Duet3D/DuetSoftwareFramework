...
deliberately no others: **what is done**, **what is left and roughly how big it is**, and **what
blocks what**. Every technical detail — why a decision was taken, how a thing is built — stays in the
plan it belongs to, linked from each row.

**The programme in one paragraph.** The Duet firmware split its work between a Linux SBC running
DuetSoftwareFramework (DSF) and a Duet main board running RepRapFirmware (RRF), talking over SPI.
That split is being removed: DSF takes over everything RRF did, `DuetCANMaster` becomes a thin
SPI-to-CAN bridge, and the expansion boards do the rest. Practically, this means porting
RepRapFirmware's G-code interpreter, motion planner, heat, tool and event subsystems into
DuetControlServer, one subsystem at a time, against the reference tree in `lib/RepRapFirmware`.
...
Workstream 7 is the umbrella the others were carved out of; it is the bulk of what is left.
Workstream 8 is fully specified and independent — it is the largest piece that can start today.
...
Sizes are **relative to each other**, not calendar estimates, and are **derived for this document**
by reading each plan's scope — the plans themselves carry no sizing. They assume one engineer already
...
without one of them being wrong. Workstream 7 dominates every count, and two items inside it — arcs,
and the height-map/`G30 P` tables — are ports of RepRapFirmware subsystems whose size is known only
...
Recorded so the remaining list is read against what it sits on, not as a fresh start.

| Workstream | What landed |
|---|---|
| **Motion config ordering** (WS1) | Tuning values travel on each move instead of being pushed as shared configuration. Removed three unnecessary machine stalls (M425, M572, M593) and a class of bug where a setting applied retroactively to queued moves. |
| **SBC motion engine cleanup** (WS2) | The RepRapFirmware compatibility scaffolding is gone from the SBC motion engine; the code reads as this project's own. One deliberate omission: the lint CI job (§6.2) was dropped by decision. |
| **Input monitors** (WS3) | All five phases. Probes are told when they are probing, abandoned monitors release their pins, held input levels are cleared. *(The summary table at the head of that plan still shows ⬜ against every phase; the per-phase sections are all ✅. Worth correcting — see §6.)* |
...
| **Job lifecycle** (WS6) | Pause, resume, cancel and stop; the restore points; `pause.g`/`resume.g`/`stop.g`/`cancel.g`/`start.g`; the feedhold (a controlled deceleration replacing RRF's search for a slow-enough junction); job progress and time estimates. |
| **M-code migration** (WS7) | 109 of 187 in-scope M-codes fully ported. Motion, kinematics (all 7 geometries), compensation, probing, heat, fans, tools and spindles are each essentially complete as subsystems. The G0/G1 audit's phases A–E, the endstop-correction move into DCS, and the kinematics ownership inversion have all landed. |
...
| Phase 4 tail — decide whether `M25.1` errors or stays an alias | S | *decision* |
...
| Filament codes M701–M703 | M | filaments directory and macros |
| M200 volumetric extrusion, M404/M407 filament width | M | reconcile one global width vs. per-extruder |
| Heat tail — M303 PID tuning (needs a state machine), M108, M144, M305, M309 | M | — |
| Multi-motion-system — M595, M596, M597, M598, M599 | L | — |
| Network tail — M540, M553, M554, M555, M575, M587–M589 | M | — |
| Miscellaneous tail — ~20 codes incl. M80/M81, M117, M150, M260/M261, M300, M905, M911/M916, M955/M956 | L | — |

M596 is worth calling out: three separate plans park a `// TODO` on it (the feedhold stops only ring
0, `RawMove.OwnedDrives` is never set, and synchronised actions belong to one ring). It is one task
that closes gaps in three places.

#### 7d — Audit findings, code rather than documentation

Found by reading the plan back against the tree. Small, and each is a live defect.
...
Doing things at a point in the path without stopping the machine — today a fan change or a servo
move mid-print forces the machine to standstill. Fully specified, nothing in front of it, and the
largest single item on the board. It spans three codebases, which is why it is sized as it is.
...
| 4–5 | Action entries in the motion service, and one submission path shared with moves | L | The mechanical heart |
...
**Four tracks can run in parallel from today:**

| Track | Contents | Why it is independent |
|---|---|---|
| **A — Synchronised actions** | All of WS8 | Nothing blocks it. Touches CAN schema and expansion firmware, so it overlaps least with the others |
| **B — Unblocking codes** | M291, M581, M452, M596 | Four medium-to-large codes that between them release every blocked tail in WS5, WS6 and parts of WS7 |
| **C — Motion pipeline** | WS7a, plus WS4 phase 8 | Self-contained DCS work; arcs are the long pole |
| **D — Probing and levelling** | WS7b | Needs machine time; `G30 P` gates the other two |

**Do first, regardless of staffing:** the four WS7d audit findings and the `M114`/object-model
position publishing. All are S, all are live defects, and one of them (positions reporting zero) is
visible to every user of the web interface.

**Track B is the highest-leverage sequencing decision.** M291 alone unblocks three tails across two
workstreams; M596 closes parked TODOs in three plans. Staffing track B early shortens the tail of
everything else.
...
Items where the plans stop short of an answer, and someone has to make a call.
...
| 3 | Nominal filament width — one global (RRF) or one per extruder (object model)? | Engineering | Blocks M200, M404 and M407 |
| 4 | Firmware emulation mode (M555) — global or per input channel? | Engineering | Blocks M555 |
| 5 | What should notice an expansion board that lost its input monitors? | Engineering | A board that resets mid-job silently loses its endstops |
...
- **Hardware verification is a shared bottleneck.** Six tasks across three workstreams are marked 🔧
  and each needs a real machine. Worth scheduling as a batch rather than per task.
- **WS8 spans three codebases** — the CAN schema, `Duet3Expansion` firmware and DCS. Its steps 1–3
  are deliberately behaviour-neutral so the risky part (steps 4–6) lands against a known-good base;
  keep that ordering.
- **Status drift in the plans.** Two instances found while writing this: WS3's summary table
  ...
  recounted. Statuses that read ✅ wrongly are the expensive direction — they are discovered at the
  machine.
- **Two RepRapFirmware refinements are deliberately not being ported** (hangprinter line buildup and
  flex compensation) because nothing in the object model can express them. Recorded so it is a
...
It is a rollup: it holds no fact of its own, and every number in it is derived from a plan in this
directory. When a phase is ticked in its own plan, update the row here in the same commit. If the two
disagree, **the plan wins** — and the plan should be trusted over any status table, including its
own summary, which is how WS3 came to show five ⬜ phases that had all landed.
