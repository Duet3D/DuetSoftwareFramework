...
public sealed class StartPlugin(PluginManager pluginManager, IHostApplicationLifetime lifetime, ILoggerFactory loggerFactory, IOptions<Settings> settings) : DuetAPI.Commands.StartPlugin
{
    ...
    public override async Task ExecuteAsync(CancellationToken cancellationToken = default)
    {
        ...
        string sbcExecutable = Path.Combine(_settings.PluginDirectory, plugin.Id, "dsf", architecture, plugin.SbcExecutable!);
        if (!File.Exists(sbcExecutable))
        {
            sbcExecutable = Path.Combine(_settings.PluginDirectory, plugin.Id, "dsf", plugin.SbcExecutable!);
            if (!File.Exists(sbcExecutable))
            {
        ...
        using (await pluginManager.LockAsync(cancellationToken))
        {
            ...
            ProcessStartInfo startInfo = new()
            {
                ...
                Arguments = (plugin.SbcPythonDependencies.Count == 0) ? plugin.SbcExecutableArguments : _settings.PythonLaunchArguments
                    .Replace("{pluginDir}", Path.Combine(_settings.PluginDirectory, plugin.Id))
