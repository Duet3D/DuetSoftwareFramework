...
// As a side effect it sets clocksNeeded. If 3rd order motion control is used it also sets the start speed and
// acceleration in the following DDA.
PrepParams DDA::BuildProfile() noexcept
{
	PrepParams params;
	params.totalDistance = m_totalDistance;
	// Due to rounding error, for an accelerate-decelerate move we may have accelDistance+decelDistance slightly greater
	// than totalDistance. We need to make sure that accelDistance <= decelStartDistance for subsequent calculations to
	// work.
#if SUPPORT_S_CURVE
	...
	params.phaseClocks[3] =
		(params.distances[3] <= (motioncalc_t)0.0) ? 0 : std::lrint(params.distances[3] / (motioncalc_t)m_topSpeed);
	...
	params.steadyClocks =
		(steadyDistance <= (motioncalc_t)0.0) ? 0 : std::lrint(steadyDistance / (motioncalc_t)m_topSpeed);
...
{
	DebugPrintf("%s %u ts=%" PRIu32 " DDA: s=%.4g",
				tag,
				(unsigned int)GetState(),
				m_afterPrepare.moveStartTime,
		 ...
		 || (m_flags.isPrintingMove == m_prev->m_flags.isPrintingMove && m_flags.xyMoving == m_prev->m_flags.xyMoving &&
			 m_flags.isNonPrintingExtruderMove ==
				 m_prev->m_flags.isNonPrintingExtruderMove // this is to prevent extruder-only move being melded with
														   // Z-axis moves (issue 990)
			 ));
	if (melded)
	{
		// Try to meld this move to the previous move to avoid stop/start
		// Assuming that this move ends with zero speed, calculate the maximum possible starting speed: u^2 = -2as
		// limited to the requested speed
		m_prev->m_beforePrepare.targetNextSpeed =
			min<float>(fastSqrtf(m_maxAcceleration * m_totalDistance * 2.0), m_requestedSpeed);
...
// A move planned as a deceleration-only move may have a short acceleration segment at the start because of rounding
// error
bool DDA::IsDecelerationMove() const noexcept
{
	return m_beforePrepare.decelDistance == m_totalDistance // the simple case - is a deceleration-only move
		   || (m_topSpeed <
				   m_requestedSpeed // can't have been intended as deceleration-only if it reaches the requested speed
...
#  define LA_DEBUG                                                                                                     \
	  do                                                                                                               \
	  {                                                                                                                \
		  if (fabsf(fsquare(laDDA->m_endSpeed) - fsquare(laDDA->m_startSpeed)) >                                       \
				  2.02 * laDDA->m_maxAcceleration * laDDA->m_totalDistance ||                                          \
...
// Only called if this move and the next one (which we have just added) are both printing moves, or both non-printing
// moves. laDDA must be provisional: a committed move's speeds are already on their way to the boards.
/*static*/ void DDA::DoLookahead(DDARing& ring, DDA* laDDA) noexcept
{
	...
	for (;;)
	{
		...
		if (laDDA->IsDecelerationMove() &&
			laDDA->m_prev->m_beforePrepare.decelDistance >
				0.0 // if the previous move has no deceleration phase then no point in adjusting it
		)
		{
		...
		// This move doesn't reach its requested speed, but either it isn't a deceleration-only move or we can't adjust
...
// This may cause a move that we intended to be a deceleration-only move to have a tiny acceleration segment at the
// start Check that the move will execute in less than 2^31 step clocks and return MovementError::ok if so
MovementError DDA::RecalculateMove(DDARing& ring) noexcept
{
	...
	if (m_beforePrepare.accelDistance + m_beforePrepare.decelDistance < m_totalDistance)
	{
		...
		// It sometimes happens that we get a very short acceleration or deceleration segment. Remove any such segments
		// by reducing the top speed to the start or end speed. Don't do this if the cause is that the top speed is very
		// low because that results in issues 989 and 994
		if (m_startSpeed >= m_endSpeed)
		{
			if (m_startSpeed + m_maxAcceleration * minimumAccelOrDecelClocks > m_requestedSpeed &&
				m_startSpeed >= m_requestedSpeed * 0.9)
			{
		...
		{
			if (m_endSpeed + m_maxAcceleration * minimumAccelOrDecelClocks > m_requestedSpeed &&
				m_endSpeed >= m_requestedSpeed * 0.9)
			{
	...
	{
		// This move has no steady-speed phase, so it's accelerate-decelerate or accelerate-only or decelerate-only
		// move. If V is the peak speed, then (V^2 - u^2)/2a + (V^2 - v^2)/2d = dist So V^2(2a + 2d) = 2a.2d.dist +
		// 2a.v^2 + 2d.u^2 So V^2 = (2a.2d.dist + 2a.v^2 + 2d.u^2)/(2a + 2d) We now always set a == d so the above
		// reduces to: V^2 = (2a.dist + v^2 + u^2)/2
		...
		else
		{
			...
			// Due to rounding errors and babystepping adjustments, we may have to adjust the acceleration or
			// deceleration slightly. It's OK to adjust maxAcceleration because if we get here then we have either an
			// acceleration or a deceleration segment, not both
			if (m_startSpeed < m_endSpeed)
			{
	...
	const float totalTime =
		(2 * m_topSpeed - m_startSpeed - m_endSpeed) / m_maxAcceleration +
		(m_totalDistance - m_beforePrepare.accelDistance - m_beforePrepare.decelDistance) / m_topSpeed;
	m_clocksNeeded = (uint32_t)totalTime;
	return (totalTime < (float)(std::numeric_limits<int32_t>::max() - 100)) ? MovementError::Ok
																			: MovementError::MoveDurationTooLong;
}

// Decide what speed we would really like this move to end at and the next move to start at, assuming we want to use the
...
void DDA::Prepare(DDARing& ring,
				  ...
				  SimulationMode simMode) noexcept
{
#if SUPPORT_S_CURVE
	PrepParams params;
	...
	// Avoid setting the move start time in the past or with very little time before it starts, because this can lead to
	// us trying to modify a segment that is already executing
	...
	if (m_prev->GetState() == Committed)
	{
		...
		// Don't allow the start of a move without input shaping (e.g. retraction/repriming) to overlap a move with
		// input shaping
	...
	if (simMode < SimulationMode::Normal)
	{
...
#endif
			{
				if (drive < move.GetTotalAxes())
				{
					...
					if (delta != 0)
					{
						...
						{
							// This is a continuous rotation axis, so we may have adjusted the move to cross the 180
							// degrees position
						...
						for (size_t i = 0; i < config.numDrivers; ++i)
						{
							...
							if (driver.IsRemote())
							{
								...
								scheduler.AddAxisMovement(params,
														  driver,
														  driverSteps,
														  Duet::Sbc::Motion::StopInputForSwitch(
															  m_stopOnInput[drive], switchIndex, driver.boardAddress),
														  m_stopOnInput[drive].stopGroup,
														  m_stopOnInput[drive].stopAction);
							}
						}
					}
				}
				else
				{
					...
					if (m_directionVector[drive] != 0.0)
					{
						...
						// gets here, so there is nothing left to check.
						{
							...
							{
								extrusionFraction +=
									m_directionVector[drive]; // accumulate the total extrusion fraction
							}

#if SUPPORT_NONLINEAR_EXTRUSION
							// Add the nonlinear extrusion correction to totalExtrusion.
							// If we are given a stupidly short move to execute then clocksNeeded can be zero, which
							...
							if (driver.IsRemote())
							{
								// The MovementLinearShaped message requires the extrusion amount in steps to be passed
								// as a float. The remote board adds the PA and handles fractional steps.
		...
		m_afterPrepare.averageExtrusionSpeed =
			(extrusionFraction * m_totalDistance * (float)stepClockRate) / (float)m_clocksNeeded;
		...
		const uint32_t canClocksNeeded = scheduler.FinishMovement(m_moveId,
																  m_afterPrepare.moveStartTime,
...
{
	// Note, for Z leadscrew adjustment moves (and any other individual motor moves that we may support in future), we
	// must not use drivesMoving, because it doesn't describe the drivers that are moving.
	return (m_flags.checkEndstops) ? move.AreDrivesStopped(m_afterPrepare.drivesMoving)
