...
std::vector<int64_t> g_samples;
std::atomic<size_t> g_sampleIndex{0};

void RecordSample(int64_t latencyNs) {
    const size_t i = g_sampleIndex.fetch_add(1, std::memory_order_relaxed);
    if (i < kMaxSamples) {
        g_samples[i] = latencyNs;
    }
}
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        std::string a = argv[i];
        if (a == "--spi-dev") config.spiDevice = needArg(i);
        else if (a == "--spi-hz") config.spiFrequency = static_cast<uint32_t>(std::stoul(needArg(i)));
        else if (a == "--spi-mode") config.spiTransferMode = std::stoi(needArg(i));
        else if (a == "--gpiochip") config.gpioChipDevice = needArg(i);
        else if (a == "--tfr-pin") config.transferReadyPin = std::stoi(needArg(i));
        else if (a == "--dap-pin") config.dataAvailablePin = std::stoi(needArg(i));
        else if (a == "--out-pin") config.sbcDataAvailablePin = std::stoi(needArg(i));
        else if (a == "--core") config.isolatedCoreId = std::stoi(needArg(i));
        else if (a == "--no-isolate") config.isolateInterfaceThread = false;
        else if (a == "--no-rt") config.useRealtimeScheduling = false;
        else if (a == "--if-prio") config.interfaceRtPriority = std::stoi(needArg(i));
        else if (a == "--rate") rateHz = std::stod(needArg(i));
        else if (a == "--msgs-per-cycle") msgsPerCycle = std::stoi(needArg(i));
        else if (a == "--dst") dstAddress = std::stoi(needArg(i));
        else if (a == "--producer-core") producerCore = std::stoi(needArg(i));
        else if (a == "--producer-prio") producerPrio = std::stoi(needArg(i));
    ...
    g_samples.assign(kMaxSamples, 0);
    ...
    std::printf("  gpio=%s tfr=%d dap=%d out=%d\n", config.gpioChipDevice.c_str(), config.transferReadyPin,
                config.dataAvailablePin, config.sbcDataAvailablePin);
    std::printf("  isolate=%d core=%d rt=%d if-prio=%d\n", config.isolateInterfaceThread,
                config.isolatedCoreId, config.useRealtimeScheduling, config.interfaceRtPriority);
    std::printf("  rate=%.0f Hz msgs/cycle=%d dst=%d producer-core=%d producer-prio=%d drop-last=%d\n\n", rateHz,
                msgsPerCycle, dstAddress, producerCore, producerPrio, dropLast);
    ...
    try {
        ...
        while (g_running.load(std::memory_order_relaxed)) {
            ...
            if (seconds > 0 &&
                std::chrono::steady_clock::now() - startTime >= std::chrono::seconds(seconds)) {
                g_running.store(false);
            }
        }
        ...
        std::vector<int64_t> samples(g_samples.begin() + dropF, g_samples.begin() + dropF + used);
