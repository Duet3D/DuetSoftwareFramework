...
public:
    ...
    explicit RingBuffer(size_t capacity) : _buffer(Align(capacity), 0), _capacity(Align(capacity)) {}
    ...
    bool Peek(const uint8_t *&data, uint32_t &length) {
        ...
        std::memcpy(&recordLength, _buffer.data() + tail, kHeaderSize);
        ...
        data = _buffer.data() + tail + kHeaderSize;
        length = recordLength;
        _pendingConsume = kHeaderSize + Align(recordLength);
        return true;
    }
    ...
    void Consume() {
        ...
        _tail.store(tail + _pendingConsume, std::memory_order_release);
        _pendingConsume = 0;
    }
    ...
    uint64_t DroppedRecords() const { return _dropped.load(std::memory_order_relaxed); }
...
private:
    ...
    size_t FreeContiguous(size_t head, size_t tail) const {
        if (head >= tail) {
            // ...tail....head------end   (free to the end)
            return _capacity - head - kHeaderSize;
        }
    ...
    bool WriteLocked(const void *const *fragments, const size_t *lengths, size_t count) {
        ...
        if (needed + kHeaderSize > _capacity) {
        ...
        if (FreeContiguous(head, tail) < needed) {
            ...
            if (head >= tail && tail > kHeaderSize && (head + kHeaderSize) <= _capacity) {
                const uint32_t marker = kSkipMarker;
                std::memcpy(_buffer.data() + head, &marker, kHeaderSize);
        ...
        const uint32_t recordLength = static_cast<uint32_t>(payload);
        std::memcpy(_buffer.data() + head, &recordLength, kHeaderSize);
        size_t cursor = head + kHeaderSize;
        for (size_t i = 0; i < count; i++) {
            if (lengths[i] > 0) {
                std::memcpy(_buffer.data() + cursor, fragments[i], lengths[i]);
                cursor += lengths[i];
            }
        }

        // Publish the record only once all of its bytes are visible to the consumer
        _head.store(head + needed, std::memory_order_release);
        return true;
    }
