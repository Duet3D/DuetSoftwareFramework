...
public class MQTT(Logger logger, IHostApplicationLifetime lifetime)
{
    ...
    private MqttClientFactory? _factory = null;
    ...
    /// </summary>
    /// <returns></returns>
    public async Task<Message> ConfigureProtocolAsync(Code code)
    {
    ...
    {
        await logger.LogOutputAsync(new Message(MessageType.Success, arg.ApplicationMessage.ConvertPayloadToString()), lifetime.ApplicationStopping);
    }
    ...
    /// <returns></returns>
    public async Task<Message> PublishAsync(Code code)
    {
        if (_client == null ||  !_client.IsConnected)
        {
