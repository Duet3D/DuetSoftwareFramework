    ...
    ILogger<GCodeHandler> logger) : ICodeHandler
{
    ...
    /// <remarks>
    /// <para>
    /// RepRapFirmware's <c>Move::AxisTransform</c>. The machine's axes are not quite at right angles
    /// to each other, so a move along one drags the head slightly along another; M556 measures that
    /// as a deviation over a distance and stores the tangent. Correcting it means adding back what
    /// the skew will take away, which is a term per pair of axes.
    /// </para>
    /// <para>
    /// The XY term goes on one axis or the other, never both - `M556 P` chooses, and correcting both
    /// would double it. Which one is a matter of which axis the machine is squared against.
    /// </para>
    /// <para>
    ...
    /// </para>
    /// </remarks>
    private void ApplyAxisSkewTransform(Span<float> coords, int numAxes)
    {
        ...
        // The axes named X and Y rather than the tool's mapping of them, as everywhere else here
        // TODO use the tool's X and Y axis mapping once there is a tool subsystem
        uint xAxes = AxisBitmap(model.Move, 'X');
        uint yAxes = AxisBitmap(model.Move, 'Y');
    ...
    /// <summary>
    /// Take M556 axis skew compensation back off a machine position
    /// </summary>
    /// <param name="coords">Machine coordinates, corrected on the way in and requested on the way out</param>
    /// <param name="numAxes">Number of axes to consider</param>
    /// <remarks>
    /// RepRapFirmware's <c>Move::InverseAxisTransform</c>, and note that it undoes the pair in the
    /// opposite order to <see cref="ApplyAxisSkewTransform"/> - Y before X, where the forward
    /// transform does X before Y. Same reason the forward one reads its references live: whichever of
    /// the two carries the cross term has to see the other in the state it was in
    /// </remarks>
    private void RemoveAxisSkewTransform(Span<float> coords, int numAxes)
    {
        Skew skew = model.Move.Compensation.Skew;
        if (skew.TanXY == 0.0f && skew.TanXZ == 0.0f && skew.TanYZ == 0.0f)
        {
