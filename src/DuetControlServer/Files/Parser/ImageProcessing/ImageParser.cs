...
public static partial class ImageParser
{
    ...
    private static partial Regex _generateBase64Regex();

    private static readonly Regex _base64Regex = _generateBase64Regex();
        ...
        while (codeParserBuffer.GetPosition(stream) < stream.Length)
        {
                ...
                trimmedComment.StartsWith("thumbnail_QOI end", StringComparison.InvariantCultureIgnoreCase))
            {
                if (readThumbnailContent)
                {
                    ...
                    thumbnail.Data = (dataContent is not null && _base64Regex.IsMatch(dataContent)) ? dataContent : null;
                }
