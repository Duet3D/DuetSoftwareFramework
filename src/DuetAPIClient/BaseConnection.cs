...
public abstract class BaseConnection(ConnectionMode mode) : IDisposable
{
    ...
    protected void Connect(ClientInitMessage initMessage, string socketPath)
    {
        ...
        ServerInitMessage serverMessage = ReceiveInitMessage();
        Id = serverMessage.Id;
        ...
        // Check the result
        BaseResponse response = ReceiveResponse();
        if (!response.Success)
        {
    ...
    /// <exception cref="SocketException">Connection has been closed</exception>
    private BaseResponse ReceiveResponse()
    {
        using MemoryStream jsonStream = JsonHelper.ReceiveUtf8Json(_unixSocket);
        //Console.Write($"IN {Encoding.UTF8.GetString(jsonStream.ToArray())}");
    ...
    /// <summary>
    /// Receive a base response from the server asynchronously
    /// </summary>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Deserialized base response</returns>
    /// <exception cref="OperationCanceledException">Operation has been cancelled</exception>
    ...
    {
        using MemoryStream json = JsonHelper.ReceiveUtf8Json(_unixSocket);
    ...
    /// <exception cref="SocketException">Connection has been closed</exception>
    private ServerInitMessage ReceiveInitMessage()
    {
        MemoryStream jsonStream = JsonHelper.ReceiveUtf8Json(_unixSocket);
        //Console.Write($"IN {Encoding.UTF8.GetString(jsonStream.ToArray())}");
    ...
    protected void SendInitMessage(InitMessage message)
    {
        ...
        _unixSocket.Send(jsonToWrite, SocketFlags.None);
    ...
    /// <exception cref="OperationCanceledException">Operation has been cancelled</exception>
    /// <exception cref="SocketException">Message could not be processed</exception>
    protected async ValueTask SendInitMessageAsync(InitMessage message, CancellationToken cancellationToken)
    {
        byte[] jsonToWrite = JsonSerializer.SerializeToUtf8Bytes(message, message.GetType(), ConnectionContext.Default);
        //Console.Write($"OUT {Encoding.UTF8.GetString(jsonToWrite)}");
    ...
    /// <exception cref="OperationCanceledException">Operation has been cancelled</exception>
    /// <exception cref="SocketException">Message could not be processed</exception>
    protected async ValueTask SendCommandAsync(BaseCommand command, CancellationToken cancellationToken)
    {
        byte[] jsonToWrite = JsonSerializer.SerializeToUtf8Bytes(command, command.GetType(), CommandContext.Default);
        //Console.Write($"OUT {Encoding.UTF8.GetString(jsonToWrite)}");
        await _unixSocket.SendAsync(jsonToWrite, SocketFlags.None, cancellationToken);
