...
class IoPort
{
  public:
	...
	static size_t AssignPorts(const char* _ecv_array pinNames,
							  const StringRef& reply,
							  PinUsedBy neededFor,
							  size_t numPorts,
							  IoPort* const _ecv_from ports[],
							  const PinAccess access[]) noexcept;
	bool AssignPort(const char* _ecv_array pinName,
					const StringRef& reply,
					PinUsedBy neededFor,
	...
	// enabled. On the SAME5x there is no debouncing on the pin itself, but the EI can deglitch or debounce before it
	// generated an interrupt. The problem with this scheme is that if we read the pin inside the ISR, the state we read
	// may not be the same as the state that caused the interrupt. This is a particular problem when the interrupt mode
	// is 'change'. It can cause lost state changes for endstops or Z probes, because if e.g. the probe makes contact
	// but bounces a bit, the contact may generate an interrupt but the ISR could read the pin as not triggered because
	// of a bounce. If that bounce is too short to pass through the debouncer/deglitcher then we don't detect that the
	// probe has triggered. We can mitigate this when using the debouncer if we read the pin state via the debounce
	// register instead.
	...
	// Warning: for speed when bit-banging Neopixels, FastDigitalWriteHigh and FastDigitalWriteLow do not take account
	// of pin inversion!
  ...
  protected:
	...
	uint8_t hardwareInvert : 1, // true if the hardware includes inversion
		...
		alternateConfig : 1 {}; // true if we are using the alternate configuration of this pin, e.g. SDADC instyead of
								// ADC

	static PinUsedBy portUsedBy[NumNamedPins];	 // the list of what each logical port is used by
	static int8_t logicalPinModes[NumNamedPins]; // what mode each logical pin is set to - would ideally be class
												 // PinMode not int8_t
};

static_assert(sizeof(IoPort) == 2,
			  "Unexpected size for class IoPort"); // try to keep these small because triggers have arrays of them
