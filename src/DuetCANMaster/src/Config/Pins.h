...
// clang-format off
// The token adjacency here is load-bearing: P_CONCAT pastes by juxtaposition, so
// "P_EXPAND(x)y" must stay unspaced. Formatted as "P_EXPAND(x) y", P_XSTR stringifies to
// "Pins_ Duet3_MB6HC .h" and the #include below cannot be resolved.
...
#define HAS_NETWORKING                                                                                                 \
	(HAS_LWIP_NETWORKING || HAS_WIFI_NETWORKING || HAS_W5500_NETWORKING || HAS_RTOSPLUSTCP_NETWORKING ||               \
	 HAS_ESP32_NETWORKING)
...
// Currently we can only attach scanning Z probes via CAN, so to save flash memory space and a small amount of RAM on
// Duet 2 we disable it
