...
constexpr float DefaultMinFeedrate = 0.5;	 // the default minimum movement speed in mm/sec (extruding moves will go
											 // slower than this if the extrusion rate demands it)
...
constexpr unsigned int DefaultBacklashCorrectionDistanceFactor =
	10; // backlash correction is spread over (backlash amount * this) mm

// We allow a slightly higher tolerance than the standard NIST values because rounding error may make the apparent
// radius a little smaller than actual
constexpr float MaxCncRadiusErrorMm = 0.0022; // max difference between G2/G3 start and end distances from arc centre
											  // when in CNC mm mode, see NIST 3.5.3.2 (NIST standard is 0.002mm)
constexpr float MaxCncRadiusErrorInches =
	0.00022; // max difference between G2/G3 start and end distances from arc centre when in CNC inches mode, see
			 // NIST 3.5.3.2 (NIST standard is 0.0002 inches)
constexpr float MaxNonCncRadiusError =
	0.05; // max difference between G2/G3 start and end distances from arc centre when not in CNC mode (mm)
...
constexpr uint16_t MinimumGpinReportInterval =
	30; // Minimum interval in milliseconds between input change reports sent over CAN bus
...
constexpr uint32_t UnsolicitedStatusReportInterval =
	2000; // Interval between sending unsolicited status reports, in milliseconds
...
constexpr unsigned int MaxBlockIndent =
	10; // maximum indentation of GCode. Each level of indentation introduces a new block.
...
// - The delta calibration points use the same static ram, but when auto-calibrating we temporarily need more to hold
// the matrices etc. as follows:
...
constexpr size_t MaxScanningProbeCalibrationPoints =
	33; // The maximum number of heights we measure when calibrating a scanning probe. Use an odd number.
...
constexpr uint32_t SdCardRetryDelay =
	20; // Number of milliseconds delay between SD transfer retries. We now double for each retry.
...
// String lengths. Try not to have too many different ones, because each one causes an instantiation of the String
// template
constexpr size_t MaxMessageLength = 256;
constexpr size_t MaxTitleLength = 61;
constexpr size_t MaxMultiplePinNamesLength =
	61; // 50 is not quite enough for 4 endstops such as !duex.e2Stop+!duex.e3Stop+!duex.e4Stop+!duex.e5Stop
...
// GCodeReplyLength is also the maximum size of a fragment of the diagnostics, for example the Tasks section, so 256
// isn't long enough on some processors
constexpr size_t GCodeReplyLength = StringLength500;   // Maximum number of characters in a GCode reply that doesn't use
													   // an OutputBuffer (ATE codes can generate long replies)
constexpr size_t FormatStringLength = StringLength500; // GCode replies are processed by Platform::MessageF which uses
													   // an intermediate buffer of this length
...
// Increased GCODE_LENGTH because M587 and M589 commands on the Duet WiFi can get very long and GCode meta commands can
// get even longer Also if HAS_SBC_INTERFACE is enabled then it needs to be large enough to hold SBC commands sent in
// binary mode, see GCodeBuffer.h
constexpr size_t MaxGCodeStringLength =
	256; // maximum number of non-comment characters in a line of GCode including the null terminator

// Define the maximum length of a GCode that we can queue to synchronise it to a move. Long enough for M150 R255 U255
// B255 P255 S255 F1 encoded in binary mode (64 bytes).
constexpr size_t ShortGCodeLength = 64;

// Output buffer length and number of buffers
// When using RTOS, it is best if it is possible to fit an HTTP response header in a single buffer. Our headers are
// currently about 230 bytes long. A note on reserved buffers: the worst case is when a GCode with a long response is
// processed. After string the response, there must be enough buffer space for the HTTP responder to return a status
// response. Otherwise DWC never gets to know that it needs to make a rr_reply call and the system deadlocks.
#if SAME70 || SAME5x
...
constexpr size_t RESERVED_OUTPUT_BUFFERS =
	4; // Number of reserved output buffers after long responses, enough to hold a status response
constexpr size_t MinimumBuffersForObjectModel =
	20; // Minimum number of free buffers we want before we start assembling a request for the object model
#elif SAM4E || SAM4S
constexpr size_t OUTPUT_BUFFER_SIZE = 256; // How many bytes does each OutputBuffer hold?
constexpr size_t OUTPUT_BUFFER_COUNT = 26; // How many OutputBuffer instances do we have?
constexpr size_t RESERVED_OUTPUT_BUFFERS =
	2; // Number of reserved output buffers after long responses, enough to hold a status response
constexpr size_t MinimumBuffersForObjectModel =
	20; // Minimum number of free buffers we want before we start assembling a request for the object model
#else
...
constexpr size_t MaxReportedAxes =
	5; // This used to be fixed at 9 but even 6 is too much for some Duet 2 configurations in RRF 3.6.0-rc.1
constexpr size_t MaxReportedAxesForPanelDue =
	6; // If the requester is PanelDue then we can return more axes because fewer fields are included
constexpr size_t maxQueuedCodes = 16; // How many codes can be queued?

#if SAME70 || SAME5x
constexpr size_t MaxTrackedObjects = 64; // How many build plate objects we track. Each one needs 12 bytes of storage,
										 // in addition to the string space on the heap.
#else
constexpr size_t MaxTrackedObjects = 32; // How many build plate objects we track. Each one needs 12 bytes of storage,
										 // in addition to the string space on the heap.
#endif
...
constexpr size_t MaxLaserPixelsPerMove =
	8; // How many S parameters you can use on a single G1 command when laser engraving

// Move system
constexpr float DefaultFeedRate = 3000.0; // The initial requested feed rate after resetting the printer, in mm/min
constexpr float MaximumG0FeedRate =
	60000.0;							// The maximum feed rate for G0 commands in mm/min, if the M203 settings permit
...
constexpr float MinArcSegmentLength =
	0.02;								   // G2 and G3 arc movement commands get split into segments at least this long
...
constexpr unsigned int SegmentsPerFulArcCalculation =
	8; // we do the full sine/cosine calculation every this number of segments

constexpr uint32_t MaxSegmentTime =
	5 * 60; // maximum time (seconds) to execute a segment, we segment moves that would take longer than this
...
constexpr uint32_t DefaultGracePeriod =
	10; // how long we wait for more moves to become available before starting movement

constexpr float DefaultNonlinearExtrusionLimit =
...
constexpr float AxisRoundingError =
	0.02; // Maximum possible error when we round trip a machine position to motor coordinates and back
...
constexpr unsigned int MaxStackDepth =
	10; // Maximum depth of stack (was 5 in 3.01-RC2, increased to 7 for 3.01-RC3, 10 for 3.4.0beta6)
...
constexpr uint32_t LaserPwmIntervalMillis =
	5; // Interval (ms) between adjusting the laser PWM during acceleration or deceleration

// I2C
// A note on the i2C clock frequency.
// On a Duet WiFi attached to a DueX5 through 160mm of ribbon cable, the cable capacitance in combination with the 4K7
// pullup resistors slows down the I2C signal rise time to about 500ns. This is above the 300ns maximum specified for
// both the ATSAM4E and the SX1509B. It appears that the slow rise time interferes with the watchdog timer resets,
// because at 200MHz clock frequency the system gets stuck in a boot loop caused by the watchdog timer going off. At
// 100kHz I2C clock frequency, these issues are rare.
constexpr uint32_t I2cClockFreq =
	100000; // clock frequency in Hz. 100kHz is 10us per bit, so about 90us per byte if there is no clock stretching
constexpr size_t MaxI2cOrModbusValues = 34; // max bytes in M260 or M261 command. Increased to 34 for NeoDriver.

// File handling
#if defined(DUET3) || defined(DUET3MINI)
constexpr size_t MAX_FILES = 20; // Must be large enough to handle the max number of concurrent web requests + file
								 // being printed + macros being executed + log file
#  if SAME70
...
#  endif
#else
constexpr size_t MAX_FILES = 10; // Must be large enough to handle the max number of concurrent web requests + file
								 // being printed + macros being executed + log file
...
constexpr size_t MaxFileReadArrayElements =
	40; // Maximum number of array elements we are allowed to read in a read() function

constexpr size_t FILE_BUFFER_SIZE = 128;

constexpr size_t MaxThumbnails =
	4; // Maximum number of thumbnail images read from the job file that we store and report
...
// MaxExpectedWebDirFilenameLength is the maximum length of a filename that we can accept in a HTTP request without
// rejecting it out of hand and perhaps warning the user of a possible virus attack. It must be at least as long as any
// web file request from DWC, which is the file path excluding the initial "0:/www" and the trailing ".gz, possibly with
// "/" prepended. As at 2020-05-02 the longest filename requested by DWC is
// "/fonts/materialdesignicons-webfont.3e2c1c79.eot" which is 48 characters long It must be small enough that a filename
// within this length doesn't cause an overflow in MassStorage::CombineName. This is checked by the static_assert below.
constexpr size_t MaxExpectedWebDirFilenameLength = MaxFilenameLength - 20;
// sizeof(literal) - 1 rather than strlen(): GCC accepts strlen in a constant expression as a
// builtin extension, clang does not, so strlen here makes every TU that includes this header
// fail to parse under clang-tidy.
static_assert(MaxExpectedWebDirFilenameLength + (sizeof(DEFAULT_WEB_DIR) - 1) + (sizeof(".gz") - 1) <=
			  MaxFilenameLength);
