...
public:
	...
	bool DataReceived() const noexcept;														// True if the SPI ISR has flagged transfer activity (vs. a task wake from new outgoing data)
	void StartNextTransfer(bool keepSequence = false) noexcept;								// Kick off the next transfer (keepSequence re-arms an un-clocked transfer without advancing the sequence number)
	...
	template<typename T> const T *ReadDataHeader() noexcept;								// Read a fixed-size data header without padding (caller then reads any trailing data)
	...
	bool WriteCANResponse(const CANResponseHeader& header, const char *_ecv_null payload) noexcept;	// Forward a received CAN message to the SBC
...
private:
	...
	unsigned int dataResendAttempts;				// consecutive data resends within the current transfer, bounds the ExchangingDataResponse retry loop
	...
	// The transfer buffers must also be in non-cached RAM. We reserve them statically rather than borrowing from a networking
	// buffer pool, because this firmware is a CAN/SBC bridge and does not run a wired Ethernet stack.
