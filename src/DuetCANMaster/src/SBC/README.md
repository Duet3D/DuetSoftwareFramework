...
## Timing And The Two-Pin Handshake

The link is fully **data-gated**: transfers happen only when at least one side has something to
send. RRF drives two GPIOs towards the SBC (the SBC is the SPI master and clocks every transfer):

- **SbcTfrReadyPin** — "RRF is armed and able to receive." Driven **high** only while RRF is idle-armed
	and waiting for the SBC to start a transfer. It is driven **low** as soon as a transfer is initiated,
	and remains low through completion/processing (and during resets/errors) until the next transfer is
	armed. RRF re-arms immediately after every transfer (no inter-transfer delay).
- **SbcDataAvailablePin** — "RRF has data to send." Driven high when the armed transfer carries payload
  (`txPointer > 0`), low when it is empty. This lets the SBC clock a transfer to pull RRF's data even
  when the SBC itself has nothing queued.

A transfer therefore occurs only when TfrReady is up **and** (the SBC has data **or** SbcDataAvailable
is asserted). At idle there are no transfers at all, so there is no throttle/backoff to tune.

When outgoing data is queued on the RRF side while it sits idle-armed with an empty buffer, the
producer wakes the SBC task, which folds the new data into the armed buffer and re-arms *without*
advancing the transfer sequence number, flipping SbcDataAvailable high so the SBC pulls it next clock.
