...
public:
	...
	void EnqueueCanTextReply(uint16_t txToken, CanRequestId requestId, const char *text) noexcept;	// Forward a text reply to the SBC as one or more standardReply CAN responses
...
private:
	...
	// Ring buffer of CAN responses waiting to be forwarded to the SBC. Producers are the CAN receiver tasks; the consumer is the SBC task.
	...
	volatile size_t canResponseHead, canResponseTail;	// head = next slot to write, tail = next slot to read; empty when equal
