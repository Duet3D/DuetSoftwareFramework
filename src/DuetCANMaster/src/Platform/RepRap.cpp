...
static_assert(configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY <= NvicPriorityHSMCI,
			  "configMAX_SYSCALL_INTERRUPT_PRIORITY is set too high");

// This is the string that identifies the board type and firmware version, that the vector at 0x20 points to.
// The characters after the last space must be the firmware version in standard format, e.g. "3.3.0" or "3.4.0beta4".
// The firmware build date/time is not included.
...
#  endif
	...
	{
		// Suspend this task until we get an interrupt indicating that a status bit that we are interested in has been
		// set
...
void RepRap::Init() noexcept
{
	...
	// sbcInterface is not initialised until we know we are using it, to prevent a disconnected SBC interface generating
	// interrupts and DMA
...
#else
	{
		...
		// The watchdogs on the SAM4E seem to be very timing-sensitive. On the Duet WiFi/Ethernet they were going off
		// spuriously depending on how long the DueX initialisation took. The documentation says you mustn't write to
		// the mode register within 3 slow clocks after kicking the watchdog. I have a theory that the converse is also
		// true, i.e. after enabling the watchdog you mustn't kick it within 3 slow clocks So I've added a delay call
		// before we set 'active' true (which enables kicking the watchdog), and that seems to fix the problem.
#  if SAM4E || SAME70
		const uint16_t mainTimeout =
			49152 /
			128; // set main (back stop) watchdog timeout to 1.5s second (max allowed value is 4095 = 16 seconds)
		WDT->WDT_MR = WDT_MR_WDRSTEN | WDT_MR_WDDBGHLT | WDT_MR_WDV(mainTimeout) |
					  WDT_MR_WDD(mainTimeout); // reset the processor on a watchdog fault, stop it when debugging

		// The RSWDT must be initialised *after* the main WDT
		const uint16_t rsTimeout =
			32768 / 128; // set secondary watchdog timeout to 1 second (max allowed value is 4095 = 16 seconds)
#	if SAME70
		RSWDT->RSWDT_MR = RSWDT_MR_WDFIEN | RSWDT_MR_WDDBGHLT | RSWDT_MR_WDV(rsTimeout) |
						  RSWDT_MR_ALLONES_Msk;				// generate an interrupt on a watchdog fault
...
#	else
		RSWDT->RSWDT_MR = RSWDT_MR_WDFIEN | RSWDT_MR_WDDBGHLT | RSWDT_MR_WDV(rsTimeout) |
						  RSWDT_MR_WDD(rsTimeout);		  // generate an interrupt on a watchdog fault
		NVIC_SetPriority(WDT_IRQn, NvicPriorityWatchdog); // set priority for watchdog interrupts
		NVIC_ClearPendingIRQ(WDT_IRQn);
...
#  else
		...
		const uint16_t timeout =
			32768 / 128; // set watchdog timeout to 1 second (max allowed value is 4095 = 16 seconds)
		wdt_init(WDT,
				 WDT_MR_WDRSTEN | WDT_MR_WDDBGHLT,
				 timeout,
				 timeout); // reset the processor on a watchdog fault, stop it when debugging
#  endif
...
// Send diagnostics to the specified destination. This is in a separate function so that the large string doesn't take
// up main task stack space all the time.
...
// Turn off the heaters, disable the motors, and deactivate the Heat, Move and GCodes classes. Leave everything else
// working.
	...
	if (m_active)
	{
		...
		if (!m_stopped)
		{
				...
				MaxMainTaskTicksInSpinState) // if we stall for 20 seconds, save diagnostic data and reset
			{
				...
				// When a task gets stuck, sometimes we want the stack of that task and sometimes we want the stack of
				// the running task instead
#if 1
...
#endif
				{
					...
					// NOLINTNEXTLINE(cppcoreguidelines-init-variables) - reads r2, must not be initialised
				...
				{
					// returns const volatile StackType_t*; this drops volatile, not const
					// NOLINTNEXTLINE(cppcoreguidelines-pro-type-const-cast) - pxTaskGetLastStackTop
					relevantStackPtr = const_cast<const uint32_t * _ecv_array>(
						pxTaskGetLastStackTop(relevantTask->GetFreeRTOSHandle()));
					// All registers were saved on the stack, so to get useful return addresses we need to skip most of
					// them. See the port.c files in FreeRTOS for the stack layouts
