...
enum MessageType : uint32_t
{
	...
	File2Message = 0x1000u,	   // A message that is to be sent to the second file processor
	Queue2Message = 0x2000u,   // A message that is to be sent to the second code queue channel
	Usb2Message = 0x4000u,	   // A message that is to be sent to the second USB channel
	MqttMessage = 0x8000u,	   // A message that is to be published by the MQTT client
	...
	DestinationsMask = 0x3FFFFu, // Mask for all the destinations
	...
	GenericMessage = UsbMessage | AuxMessage | HttpMessage |
					 TelnetMessage,				   // A message that is to be sent to the web, Telnet, USB and panel
	...
	FirmwareUpdateErrorMessage =
		FirmwareUpdateMessage | ErrorMessageFlag, // A message that reports an error during a firmware update
	NetworkInfoMessage = UsbMessage | AuxMessage |
						 LogWarn, // A message that conveys information about the state of the network interface
	NetworkErrorMessage =
		UsbMessage | AuxMessage | LogWarn |
