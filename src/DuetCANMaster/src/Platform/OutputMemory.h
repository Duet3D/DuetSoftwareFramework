...
class OutputBuffer
{
  public:
	...
	// Allocate an unused OutputBuffer instance, optionally from the reserve. Returns true on success or false if no
	// instance could be allocated.
	static bool Allocate(OutputBuffer* _ecv_null& buf, bool allowReserved = true) noexcept;

	// Get the number of bytes left for allocation. If writingBuffer is not NULL, this returns the number of free bytes
	// for continuous writes, i.e. for writes that need to allocate an extra OutputBuffer instance to finish the
	// message.
	...
	static void ReleaseAll(OutputBuffer* volatile _ecv_null& buf) noexcept;
