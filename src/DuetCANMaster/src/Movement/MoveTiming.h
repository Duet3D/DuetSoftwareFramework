...
namespace MoveTiming
{
	...
	constexpr uint32_t MinCalcInterval =
		(40 * StepClockRate) /
		1000000; // the smallest sensible interval between calculations (40us) in step timer clocks
...
#  error Unsupported processor
#endif
	constexpr uint32_t MinInterruptInterval =
		(6 * StepClockRate) / 1000000; // Minimum interval between step timer interrupts, in step clocks; about 6us. See
									   // StepTimer::ScheduleTimerInterrupt.
	constexpr uint32_t MaxStepInterruptTime =
		10 * MinInterruptInterval;						 // the maximum time we spend looping in the ISR, in step clocks
	...
	constexpr uint32_t NominalRemoteDriverPositionUpdateInterval =
		StepClockRate / 10; // how often we aim to generate an interrupt to update the position of remote drivers
	constexpr uint32_t MaxRemoteDriverPositionUpdateInterval =
		(3 * NominalRemoteDriverPositionUpdateInterval) /
		2; // the maximum interval between interrupts to update the position of remote drivers
#endif
} // namespace MoveTiming
