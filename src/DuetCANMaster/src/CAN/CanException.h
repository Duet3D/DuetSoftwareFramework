...
{
public:
	CanException() noexcept : message(nullptr) { }
	explicit CanException(const char *_ecv_array msg) noexcept: message(msg) { param.i = 0; }

	CanException(const char *_ecv_array msg, const char *_ecv_array sparam) noexcept : message(msg)
	{
		stringParam.copy(sparam);
	}

	CanException(const char *_ecv_array msg, uint32_t uparam) noexcept : message(msg)
	{
		param.u = uparam;
	}

	CanException(const char *_ecv_array msg, int32_t iparam) noexcept : message(msg)
	{
		param.i = iparam;
	}
...
// Functions to create and throw an exception. Using these avoids allocating the CanException object on the local stack when it is not going to be used.
[[noreturn]] void __attribute__((noinline)) ThrowCANException(const char *_ecv_array errMsg) THROWS(CanException);
[[noreturn]] void __attribute__((noinline)) ThrowCANException(const char *_ecv_array errMsg, uint32_t param) THROWS(CanException);
