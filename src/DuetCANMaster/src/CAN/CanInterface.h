...
namespace CanInterface
{
	...
	GCodeResult RemoteDiagnostics(MessageType mt, uint32_t boardAddress, unsigned int type, const StringRef& reply) THROWS(CanException);
	...
	// 'txToken' is the SBC's token to return in any response; 'replyType' is the reply the SBC expects (0xFFFF means none).
	void SendCanRequest(CanMessageBuffer& buf, uint16_t txToken, CanMessageType replyType) noexcept;
	CanRequestMapping *_ecv_null FindPendingRequest(CanAddress src, CanRequestId rid) noexcept;	// Find an in-flight request matching a received response
	void ReleasePendingRequest(CanRequestMapping *mapping) noexcept;							// Free a pending request slot and any reassembly buffer
	...
	void EnableCan(bool enable) noexcept;										// enable or disable the CAN interface (enabling starts the time sync broadcasts)
	void ConfigLocalCanTiming(const CanTiming& timing, bool doSetTiming, const StringRef& reply) noexcept;	// configure our own CAN timing, or report the current timing into 'reply' if doSetTiming is false
