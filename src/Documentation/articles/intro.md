...
- **DuetControlServer (DCS)** is the heart of DSF. It owns the [object model](object-model.md),
  ...
  hardware.
- **DuetRealtimeCore** is a native shared library (`libduet_sbc.so`) loaded into the DCS process. It
...
| DCS | DuetRealtimeCore | P/Invoke into `libduet_sbc.so`, plus a ring buffer of inbound events |
