...
REQUIRED_BINARIES=(xmllint jq dotnet)
# DWC is the only package built from JavaScript sources
if [ -z "$PACKAGES" ] || [[ "$PACKAGES" == *dwc* ]] ; then
...
for pt in $PACKAGE_TYPES ; do
	if [ $AOT -eq 1 ] && [ $pt != "deb" ] ; then
		echo "AOT builds are only supported for deb packages, skipping $pt."
		continue
	fi
