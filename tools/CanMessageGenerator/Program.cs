...
public static class Program
{
    public static int Main(string[] args)
    {
        ...
        [
            (options.GetValueOrDefault("cpp-out") ?? Path.Combine(repoRoot, "tools/CanMessageGenerator/generated/cpp/CanMessageFormats.h"),
             cpp.Emit()),
            // The struct and union files must be produced before the buffer file, because emitting them
            // is what registers the inline array types that the buffer file declares
    ...
    /// <summary>Deep-copy a member so that expanding a template does not disturb the template itself.</summary>
    private static MemberDef CloneMember(MemberDef m) => new()
    {
        ...
        {
            Name = f.Name, Width = f.Width, Bool = f.Bool, Signed = f.Signed, Reserved = f.Reserved, Doc = f.Doc
        })],
