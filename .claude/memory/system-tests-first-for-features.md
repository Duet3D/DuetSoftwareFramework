...
**How to apply:** Start a feature by adding scenarios under `src/SystemTests/Scenarios`, driving real G-code through the hosted DuetControlServer against the scripted controller, and asserting on the object model and the captured link traffic. Only then implement, until the scenarios pass. Run them with `./scripts/system-tests.sh --filter "FullyQualifiedName~<the new fixture>"`, never the whole suite ([[no-full-system-test-suite]]). A scenario the source does not satisfy yet, because the behaviour is unimplemented or implemented incorrectly, carries `[Category("KnownGap")]`, which keeps it out of a default run until the source is fixed and the category is taken off. Hardware checks ([[pi-hardware-test-workflow]]) come after, for what the bench cannot reach.
